...
#
# OPTION A — different destinations per source (edit FORWARD_MAP directly):
FORWARD_MAP: dict[int, list[int]] = {
    # -1001111111111: [-1002222222222],                # Group A -> one destination
    # -1003333333333: [-1004444444444, 987654321],     # Group B -> a group AND a DM
}

# OPTION B — many source groups, all forwarding to the SAME destination(s).
...
for _source in COMMON_SOURCE_GROUPS:
    ...
    for _dest in COMMON_DESTINATIONS:
        if _dest not in FORWARD_MAP[_source]:
            FORWARD_MAP[_source].append(_dest)
